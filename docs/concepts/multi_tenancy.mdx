...
_GitOps is a way of managing your infrastructure and applications so that whole system is described declaratively and version controlled (most likely in a Git repository), and having an automated process that ensures that the deployed environment matches the state specified in a repository._
...
This allows to define permissions depending on the role.
...
Lizz follows the multi-tenant structure and considere each application is associated to a tenant.
...
In that case, it is possible to give to an application access to all namespaces by adding the `--cluster-role` flag to the `lizz add` command.
