...
The [cluster-issuer application](https://github.com/Openlizz/application-cluster-issuer) is couple with the cert-manager application and installed the [cluster issuer](https://cert-manager.io/docs/configuration/) of your choice depending of the `--path` used.
...
```
lizz add github --owner=$GITHUB_USER  --fleet=fleet --origin-url=https://github.com/openlizz/application-smtp-server --path=./ --destination=smtp --set-value host=smtp.sendgrid.net,secure=ssl,port=465,domain=openlizz.com,smtpUsername=apikey,smtpPassword=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
``` -->
...
You can not use the ACME production server with [nip.io] but can if you have you own domain name. See info [here](#add-the-cert-manager-and-cluster-issuer-applications).
